...
[sales@codestream.com](mailto:sales@codestream.com) to request access to the
docker images. We'll need your [Docker Hub](https://hub.docker.com) user ID to
grant you access to our docker images.

## Quick Start

1.	Setup a linux server or VM with
...
1.	Login to your docker hub registry account. The host OS should cache your ID
	and password so you only need to do this once.
	```
	docker login
	```

1.	Cut and paste these commands into your shell. They will download and run the
	control script which will install and startup CodeStream.
	```
	mkdir ~/.codestream
	cd ~/.codestream
